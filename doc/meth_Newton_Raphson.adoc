...
We want to stem:[f_i \left( {x}^{(1)} \right) = 0], so the successive approximations to the solution ate obtained from
...
and stem:[J^{(k)}] is the Jacobian matrix in the k-th iteration

[stem]
++++
\left(J^{(k)} \right)_{ij} = \frac{\partial f^{(k)}_i}{\partial x^{(k)}_j}
++++
