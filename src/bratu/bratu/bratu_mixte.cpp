...
makeOptions()
{
    Feel::po::options_description bratuoptions( "Bratu problem mixte options" );
...
main( int argc, char** argv )
{
    ...
    auto q = expr( soption(_name="functions.q"), "q" ); //Dirichlet condition homogene
    ...
    auto Jacobian = [=](const vector_ptrtype& X, sparse_matrix_ptrtype& J)
        {
            ...
            //a +=on(_range=markedfaces(mesh,"Dirichlet"), _element=u, _expr=q );
    ...
    auto Residual = [=](const vector_ptrtype& X, vector_ptrtype& R)
        {
            ...
            //r += integrate(_range=markedfaces(mesh,"Dirichlet"), _expr=q );
